...
#import "MiViewPagerVC.h"

@interface MiViewPagerVC ()<UIPageViewControllerDelegate,UIPageViewControllerDataSource>
{
    ...
    MiViewPagerTitleButton *oldButton;
...
-(void)setDataSource:(id<MiViewPagerVCDataSource>)dataSource
{
    ...
    if ([self.dataSource respondsToSelector:@selector(numberOfViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<numberOfViewController; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexOfViewControllers:)]) {
                ...
                {
                    button = [[MiViewPagerTitleButton alloc] init];
                }
        ...
        if (mutableArrayOfBtn.count && ((UIButton *)mutableArrayOfBtn.lastObject).frame.origin.x + ((UIButton *)mutableArrayOfBtn.lastObject).frame.size.width<self.view.frame.size.width) //当所有按钮尺寸小于屏幕宽度的时候要重新布局
        {
            ...
            for (MiViewPagerTitleButton *button in mutableArrayOfBtn) {
...
-(void)p_titleButtonClick:(MiViewPagerTitleButton *)sender
{
...
-(void)p_titleButtonConvert:(MiViewPagerTitleButton *)sender
{
    oldButton.selected = NO;
    sender.selected = YES;
    oldButton = sender;
...
@implementation MiViewPagerTitleButton
- (instancetype)init
{
    self = [super init];
    if (self) {
...
-(void)drawRect:(CGRect)rect
{
    if (self.selected) {
        ...
        CGContextMoveToPoint(ctx, 12, self.frame.size.height-lineWidth);
        CGContextAddLineToPoint(ctx, self.frame.size.width - 12, self.frame.size.height-lineWidth);
